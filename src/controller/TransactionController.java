...
public class TransactionController {
    ...
    public void loadPurchaseHistory(PurchaseHistoryView purchaseHistoryView, User currentUser) {
        ...
        try {
            String query = "SELECT t.transaction_id, i.item_name, t.price, t.transaction_date " +
                           "FROM transactions t " +
    ...
    public void handlePurchaseItem(String itemId, User buyer) {
        try {
            ...
            String checkItemQuery = "SELECT * FROM items WHERE item_id = ? AND status = 'approved' FOR UPDATE";
            ...
            String sellerId = itemRs.getString("seller_id");
            double price = itemRs.getDouble("price");

            // 2. Create transaction record
            String transactionId = UUID.randomUUID().toString();
            String insertQuery = "INSERT INTO transactions (transaction_id, buyer_id, seller_id, item_id, price, transaction_date) VALUES (?, ?, ?, ?, ?, NOW())";
            ...
            insertPs.setString(3, sellerId);
            insertPs.setString(4, itemId);
